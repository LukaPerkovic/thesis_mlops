...
class DatabricksModelRegistry:
    def __init__(self):
        ...
        self.experiment_name = f"/Users/{os.getenv("DBX_USER")}/fraud_detection_model_registry_experiment"
    ...
    def _log_model(self, model: Model, score: tuple, name: str, sample: DataFrame):
        ...
        with mlflow.start_run(experiment_id=experiment_id) as run:
            mlflow.xgboost.log_model(xgb_model=model, name=name, input_example=sample.iloc[[0]])
    ...
    def push_model(self, model: Model, score: tuple, name: str, sample: DataFrame):
        ...
        return self._register_model(model_uri, f"{self.catalog_name}.{self.schema_name}.{name}", run_id)
